...
import InventoryManager from '../components/ai/InventoryManager';
import SustainabilityDashboard from '../components/ai/SustainabilityDashboard';
import MealPlanner from '../components/ai/MealPlanner';
import CommunityHub from '../components/ai/CommunityHub';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="flex relative">
        {/* Desktop Sidebar */}
        <div className="hidden md:flex fixed left-0 top-16 w-64 h-[calc(100vh-4rem)] bg-emerald-900 text-white flex-col shadow-xl z-20">
          ...
          <nav className="flex-1 p-4 space-y-2">
            {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === item.id ? 'bg-emerald-700 text-white shadow-lg' : 'text-emerald-100 hover:bg-emerald-800'
                  }`}
              >
        ...
        {/* Main Content Area */}
        <main className="flex-1 md:ml-64 p-4 md:p-8 lg:p-10 pb-24 md:pb-8">
          {/* Mobile Header */}
